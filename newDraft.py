...
# Create button (top-right) — approximate ref coords for "Create" button
CREATE_X = 1120
CREATE_Y = 180
...
def click_create():
    print(f"    → Clicking CREATE button at ref({CREATE_X}, {CREATE_Y})")
    click(CREATE_X, CREATE_Y, "Create", delay=2.0)
...
def process_one_edit(index, row_on_page, title, category, rarity):
    ...
    # 1. Click the Create button to open a NEW edit panel
    # (Do NOT click list rows or navigate pages in Create-only mode)
    click_create()
...
print()
print("  ⚠  NOTE: This script is now in CREATE-ONLY mode:")
print("     - It will click the top-right Create button for each item")
print("     - It will NOT click list rows or advance pages")
print()
print("  ℹ  Coord note:")
...
for i in range(total):
    process_one_edit(
        index       = i,
        row_on_page = 0,  # unused in create-only mode
        title       = titles[i],
